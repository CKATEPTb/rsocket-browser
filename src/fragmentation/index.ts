 ...
 *
 * The WebSocket transport carries raw RSocket frames without an additional
 * transport length prefix, so this module can split oversized payload-bearing
 * frames by calculating their serialized size from fixed protocol overhead.
 */
import {
  FrameFlag,
  FrameType,
...
/** Serialized RSocket frame header length for WebSocket transport frames. */
 ...
 */
export interface OutboundPayloadParts {
...
export function outboundFrameLength(frame: Frame): number | undefined {
  ...
  return frameOverhead(frameType, metadata !== undefined) +
...
export function emitOutboundFrameFragments(
  ...
  emitFragmentFrames(frame, frameLength, maxFrameLength, parts ?? outboundPayloadParts(frame), emit);
}
...
function emitFragmentFrames(
  ...
  while (metadataOffset < metadataTotal || dataOffset < dataTotal) {
    ...
    const includeMetadata = metadataRemaining > 0 || (first && parts.metadataMimeType !== undefined);
    let metadataLength = 0;
    let dataLength = 0;

    if (includeMetadata) {
      const metadataCapacity = maxFrameLength - metadataOverhead;
      if (metadataCapacity <= 0) throw new RSocketFrameSizeError(frameLength, maxFrameLength);
...
function initialFragment(
  ...
  const flags = fragmentInitialFlags(header.flags, follows, frameType);
  ...
  switch (frameType) {
    ...
    case FrameType.PAYLOAD:
      return new PayloadFrame(streamId, fragmentPayloadFlags(header.flags, follows), metadata, payload);
    default:
      throw new RSocketFrameSizeError(frameLength, maxFrameLength);
  }
}
...
function continuationFragment(
  ...
  const flags = frameType === FrameType.PAYLOAD
    ? fragmentPayloadFlags(header.flags, follows)
    : continuationPayloadFlags(frameType, header.flags, follows);
 ...
 */
function fragmentInitialFlags(flags: number, follows: boolean, type: FrameType): number {
 ...
 */
function continuationPayloadFlags(type: FrameType, originalFlags: number, follows: boolean): number {
  let flags = PayloadFlag.NEXT;
 ...
 */
function fragmentPayloadFlags(flags: number, follows: boolean): number {
  const baseFlags = flags & ~FrameFlag.METADATA & ~PayloadFlag.FOLLOWS;
  return follows ? (baseFlags & ~PayloadFlag.COMPLETE) | PayloadFlag.FOLLOWS : baseFlags;
}
