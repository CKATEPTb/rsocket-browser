 ...
 *
 * These helpers are internal building blocks for the browser client and
 * declarative controller layer. The package root intentionally exports only the
 * high-level `RSocket` class and declarative controller definitions.
 */
import {
  Metadata,
  MimeType,
 ...
 *
 * A non-composite entry is wrapped automatically when the connection uses
...
/**
 * Serializes outbound data while working around codec dispatch treating `null` as a reader.
 */
function encodePayloadValue<T>(value: T, mimeType: MimeType<T>): Payload<T> {
  const payload = value === null
    ? outboundMimeType(mimeType).serializePayload(value)
    : mimeType.toPayload(value);
  return cachedPayload(payload);
}

/**
 * Serializes outbound metadata while working around codec dispatch treating `null` as a reader.
 */
function encodeMetadataValue<T>(value: T, mimeType: MimeType<T>): Metadata<T> {
  const frameMetadata = value === null
    ? outboundMimeType(mimeType).serializeMetadata(value)
    : mimeType.toMetadata(value);
  return cachedMetadata(frameMetadata);
}
...
/**
 * Exposes the protected outbound codec methods through a narrow internal view.
 */
function outboundMimeType<T>(mimeType: MimeType<T>): {
  serializePayload(value: T): Payload<T>;
  serializeMetadata(value: T): Metadata<T>;
} {
  return mimeType as unknown as {
    serializePayload(value: T): Payload<T>;
    serializeMetadata(value: T): Metadata<T>;
  };
}
 ...
 */
function isCodecPayloadShape(value: unknown): value is { payload: unknown; mimeType: MimeType<any>; toUint8Array(): Uint8Array } {
  if (typeof value !== "object" || value === null) return false;
...
function cachedPayload<T>(source: Payload<T>): Payload<T> {
  return source instanceof CachedPayload ? source : new CachedPayload(source);
}
...
function cachedMetadata<T>(source: Metadata<T>): Metadata<T> {
  return source instanceof CachedMetadata ? source : new CachedMetadata(source);
}
